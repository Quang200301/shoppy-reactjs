...
const Adidasnam = () => {
    const [adidasnam,setAdidasnam]=useState(data);
    useEffect(()=>{
        const categoryadidasnam=adidasnam.filter((item)=>item.category === 'adidasnam');
        setAdidasnam(categoryadidasnam);
    },[])
    return (
        <div>
            <Container>
                <h4 className='productnike'>-ADIDAS BOY</h4>
                <Row>
                    {adidasnam.map((item, key) => (
