...
const Adidasnu = () => {
    const [adidasnu,setAdidasnu]=useState(data);
    useEffect(()=>{
        const categoryadidasnu =adidasnu.filter((item)=>item.category === 'adidasnu');
        setAdidasnu(categoryadidasnu);
    },[]);
    return (
        <div>
             <Container>
                <h4 className='productnike'>-ADIDAS GIRL</h4>
                <Row>
                    {adidasnu.map((item, key) => (
